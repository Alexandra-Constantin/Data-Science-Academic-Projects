...
def apply_tsne(X, n_components, perplexity, max_iter, init, random_state=2024):
    ...
    # Scale the data and apply t-SNE
...
def plot_tsne_projection(X_tsne_2d, color):
    ...
    plt.scatter(X_tsne_2d[:, 0], X_tsne_2d[:, 1], c=color, s=8)
    plt.title("Swiss Roll: 2D t-SNE projection")
    plt.xlabel("t-SNE 1")
    plt.ylabel("t-SNE 2")
    plt.tight_layout()
    plt.show()
...
def return_identified_letter():
    ...
    return "C" # closest thing to a letter I could find
